...
implements OnInit, OnDestroy, ControlValueAccessor {
  ...
  @Output() public selected = new EventEmitter<{ data: any, panelClosed: boolean }>();
    ...
    if (!this._model.includes(value)) {
      switch (selected.type) {
        case DataType.Object:
          this._addObject(selected);
          this.selected.emit({ data: selected, panelClosed: closePanel });
          break;

        case DataType.Text:
          this._addText(selected.data);
          this.selected.emit({ data: selected.data, panelClosed: closePanel });
          break;
      }
    }
